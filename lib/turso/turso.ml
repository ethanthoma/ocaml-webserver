...
let get_blogs () =
  ...
  let json =
    ...
    if str_json <> "" then Some (Yojson.Basic.from_string str_json) else None
  in
  match json with
  | Some json -> (
      let list_opt =
      ...
      in
      match list_opt with Some list -> List.map json_to_blog list | None -> [])
  | None -> []

let get_blog_by_slug slug =
  let get_blog_by_slug =
    foreign "GetBlogBySlug" (string @-> returning string)
  in
  let json =
    let str_json = get_blog_by_slug slug in
    if str_json <> "" then Some (Yojson.Basic.from_string str_json) else None
  in
  match json with Some json -> json_to_blog json | None -> empty_blog
