...
let search query ~items =
  ...
  let compare_blog a b = compare_string a.title b.title in
...
let query name =
  ...
  Fuzzy_search.Query.create name |> search ~items:blogs

let content request =
  let search_value =
    match Dream.query request "search" with Some value -> value | None -> ""
  in
  search_value |> query
